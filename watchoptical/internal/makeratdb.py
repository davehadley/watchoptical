...
def makeratdb(attenuation: Optional[float]=None, scattering: Optional[float]=None) -> Optional[str]:
    snippets = []
...
def _scatteringjson(scatteringlength: float) -> str:
    ...
    rslength2 = json.dumps([scatteringlength]*len(_nominal_doped_water_ABSLENGTH_value1))
    return inspect.cleandoc(f"""
    {{
      name: "OPTICS",
      index: "doped_water",
